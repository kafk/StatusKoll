...
const BottomNav = ({ activePage, onPageChange }: BottomNavProps) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-card border-t border-border py-3 z-[100] shadow-[0_-2px_10px_rgba(45,52,54,0.08)]">
      <div className="max-w-[428px] mx-auto flex justify-around items-center">
        {navItems.map(({ id, label, icon: Icon }) => (
          <button
            key={id}
            onClick={() => onPageChange(id)}
            className={`flex flex-col items-center gap-1 px-5 py-2 rounded-xl transition-all hover:bg-primary/10 ${
